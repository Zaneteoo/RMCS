...
<title>Reviewer View to Review</title>
</head>
		...
		<input type="text" name="search"
			...
			class="search block p-4 pl-10 w-full text-sm text-gray-900 bg-gray-50 rounded-lg border border-gray-300 focus:ring-blue-500 focus:border-blue-500 dark:bg-gray-700 dark:border-gray-600 dark:placeholder-gray-400 dark:text-white dark:focus:ring-blue-500 dark:focus:border-blue-500" />
				...
				try {
					...
					if (query != null) {
						data = "select paper_id, first_name, papername, status, allocationrev from papers WHERE allocationrev = '"
						+ uName + "'" + " AND papername LIKE" + "'%" + query + "%'";
					} else
						data = "select paper_id, first_name, papername, status, allocationrev from papers WHERE allocationrev = '"
						+ uName + "' ORDER BY FIELD (status, 'Pending', 'Accept', 'Reject'), papername asc";
					rs = stat.executeQuery(data);
					if (rs.next() != true) {
				%>
				<div>
					<label>Test</label> 
					<a class="btn" href="reviewerHome.jsp">Return to Home </a>
				</div>
				<%
				}
				rs.beforeFirst();
				...
				<table class="w-full text-sm text-left text-gray-500 dark:text-gray-400">
					<thead class="text-xs text-gray-700 uppercase bg-gray-50 dark:bg-gray-700 dark:text-gray-400">
						<tr>
							...
							<th scope="col" class="py-3 px-6">Allocated To</th>
				...
				%>
					<tbody>
						<tr class="bg-white border-b dark:bg-gray-900 dark:border-gray-700">
							...
							<td class="py-4 px-6"><%=rs.getString(5)%></td>
							<td class="py-4 px-6"><a class="text-blue-900 no-underline hover:underline" href="leavereview.jsp?id=<%=rs.getString(1)%>"> Leave Review</a> | <a class="text-blue-900 no-underline hover:underline" href="viewReview.jsp?id=<%=rs.getString(1)%>"> View Review </a></td>
							<td class="py-4 px-6"><a class="text-blue-900 no-underline hover:underline" href="viewComment.jsp?id=<%=rs.getString(1)%>&appid=<%=rs.getString(3)%>"> Comments </a></td>
						</tr>
